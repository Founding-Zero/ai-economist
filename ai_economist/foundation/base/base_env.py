...
class BaseEnvironment(ABC):
    ...
    def __init__(
        ...
        n_planners = 1
        self.num_agents = n_agents + n_planners  # used in the warp_drive env wrapper (+ 1 for the planner)
        ...
        for component_cls, component_kwargs in component_classes:
            component_object = component_cls(
                self.world, self._episode_length, inventory_scale=self.inv_scale, **component_kwargs
            )
    ...
    def _package(obs_dict, keep_as_is, flatten, wrap_as_list):
        ...
        if len(flatten) == 1:
            ...
            to_flatten = [[obs_dict[k]] if wrap_as_list[k] else obs_dict[k] for k in flatten]
            try:
    ...
    def _generate_observations(self, flatten_observations=False, flatten_masks=False):
        def recursive_listify(d):
            ...
            for k, v in d.items():
                if isinstance(v, dict):
                    ...
                    raise NotImplementedError("Not clear how to handle {} with type {}".format(k, type(v)))
        ...
        agent_wise_planner_obs = {"p" + str(agent.idx): {} for agent in self.world.agents}
        ...
        for idx, o in world_obs.items():
            if idx in obs:
                ...
                if self.collate_agent_step_and_reset_data and idx == "a":
                    obs[idx]["time"] = np.array([self.world.timestep / time_scale for _ in range(self.world.n_agents)])
                else:
                    obs[idx]["time"] = [self.world.timestep / time_scale]
            elif idx in agent_wise_planner_obs:
                agent_wise_planner_obs[idx].update({"world-" + k: v for k, v in o.items()})
            else:
                raise KeyError
        ...
        for component in self._components:
            for idx, o in component.obs().items():
                if idx in obs:
                    ...
                    agent_wise_planner_obs[idx].update({component.name + "-" + k: v for k, v in o.items()})
                else:
                    raise KeyError
            ...
            for o_dict in [obs, agent_wise_planner_obs]:
                for aidx, aobs in o_dict.items():
                    ...
                    if aidx not in self._packagers:
                        self._packagers[aidx] = self._build_packager(aobs, put_in_both=["time"])
                    try:
        ...
        for k, v in agent_wise_planner_obs.items():
            if len(v) > 0:
                obs[self.world.planner.idx][k] = v["flat"] if flatten_observations else v
            ...
            for idx, mask in component_masks.items():
                if isinstance(mask, dict):
                    for sub_action, sub_mask in mask.items():
                        masks[idx]["{}.{}".format(component.name, sub_action)] = sub_mask
                else:
        ...
        if flatten_masks:
            if self.collate_agent_step_and_reset_data:
                ...
                for agent_id in masks.keys():
                    ...
                    flattened_masks[agent_id] = np.concatenate(list_of_masks, axis=0).astype(np.float32)
                return flattened_masks
            return {str(agent.idx): agent.flatten_masks(masks[agent.idx]) for agent in self.all_agents}
        return {
            str(agent_idx): {k: np.array(v, dtype=np.uint8).tolist() for k, v in masks[agent_idx].items()}
    ...
    def _finalize_logs(self):
        ...
        def recursive_cast(d):
            ...
            if isinstance(d, (np.ndarray, np.integer, np.floating)):
                return d.tolist()
            raise NotImplementedError("Not clear how to handle {} with type {}".format(d, type(d)))

        self._dense_log["world"].append(deepcopy(self.world.maps.state_dict))
        self._dense_log["states"].append({str(agent.idx): deepcopy(agent.state) for agent in self.all_agents})
    ...
    def collate_agent_obs(self, obs):
        ...
        for key in obs["0"].keys():
            obs["a"][key] = np.stack([obs[str(agent_idx)][key] for agent_idx in range(num_agents)], axis=-1)
    ...
    def reset(self, seed_state=None, force_dense_logging=False):
        ...
        if force_dense_logging:
            ...
            self._dense_log_this_episode = (self._completions % self._create_dense_log_every) == 0
    ...
    def step(self, actions=None, seed_state=None):
        ...
        self._replay_log["step"].append(dict(actions=actions, seed_state=np.random.get_state()))
        ...
        if self._dense_log_this_episode:
            ...
            self._dense_log["actions"].append(
                {str(agent.idx): {k: v for k, v in agent.action.items() if v > 0} for agent in self.all_agents}
            )
        ...
        if self._dense_log_this_episode:
            self._dense_log["endowments"].append(
                {str(agent.idx): agent.total_endowment("Coin") for agent in self.world.agents}
            )

        for agent in self.all_agents:
            agent.reset_actions()

        if done["__all__"]:  # Complete the dense log and stash it as well as the metrics
            self._finalize_logs()
            self._completions += 1
        ...
        # info["tax_rates"] = self._components[-1].curr_marginal_rates
        # raise Exception(f"{info}") from ImportError
